...
public class ClubPadelDao {
    ...
    public void add(ClubPadel clubPadel) throws SQLException, Exception{
        try{
            ...
            if (existClubPadel(clubPadel.getNombre_club()))
                throw new Exception("Este Club ya existe");
    ...
    public boolean modify(String nombreClub, ClubPadel clubPadel) throws SQLException {
        try{
            String sql = "UPDATE CLUB_DE_PADEL SET DIRECCION = ?, TELEFONO = ?, WEB = ?, FEDERACION = ?  WHERE nombreClub = ?";
            ...
            statement.setString(5, nombreClub);
    ...
    private boolean existClubPadel(String nombreClub) throws SQLException {
        Optional<ClubPadel> clubPadel = findByName(nombreClub);
        return clubPadel.isPresent();
    }
}
